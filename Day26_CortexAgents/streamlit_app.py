...
USE WAREHOUSE COMPUTE_WH;"""
    ...
    if st.button(":material/play_arrow: Run Step 1", key="run_step1", use_container_width=True):
        with st.spinner("Creating database and schema..."):
            try:
                # for sql in [f'CREATE OR REPLACE DATABASE "{db_name}"', f'USE DATABASE "{db_name}"', f'CREATE OR REPLACE SCHEMA "{db_name}"."{schema_name}";',f'USE SCHEMA "{schema_name}"', "USE WAREHOUSE COMPUTE_WH"]:
                for sql in [f'CREATE OR REPLACE DATABASE "{db_name}"', f'CREATE OR REPLACE SCHEMA "{db_name}"."{schema_name}"', "USE WAREHOUSE COMPUTE_WH"]:
...
"""
    ...
    if st.button(":material/verified: Check if Data is Ready", type="primary", use_container_width=True):
        with st.status("Verifying setup...", expanded=True) as status:
            all_good = True
            checks = [
                (f""" SELECT * FROM INFORMATION_SCHEMA.DATABASES WHERE DATABASE_NAME = '{db_name}'""","Database exists"),
